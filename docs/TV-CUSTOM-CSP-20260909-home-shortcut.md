# TV-CUSTOM-CSP-20260909：站点注入总开关前置到首页

## Recovery anchor

- 目标：让 TV 用户从首页通过一个可聚焦的快捷按钮启用/禁用“增强功能 → 站点注入”的全局开关，避免进入设置页多层操作。
- 验收：TV 首页出现可配置的“站点注入”按钮；点击复用 `CustomCspSetting` 的同一 `registry.json` 全局 `enabled` 字段；保存后重新加载当前 VOD/直播配置并刷新按钮状态；手机端行为不变。
...
- 已完成：用户目标纠正、站点注入开关与配置重载路径定位、方案决策、task guard 启动。
- 当前待办：记录最终验证结果，原子提交并创建恢复 tag。
- 未验证风险：当前无 Android TV 设备；真实遥控器焦点、文件权限弹窗和首页按钮在不同屏幕宽度下的视觉效果需设备验证。
- 回滚锚点：本任务 guard 基线 HEAD；回滚仅还原 TV 首页 UI 与一个既有站点注入保存辅助方法，无数据迁移、依赖或 native 变更。
- 下一步：执行 `bash .codex/scripts/task_guard.sh finish` 完成本任务提交和恢复 tag。
...
因此快捷入口必须保存 `registry.json` 的 `enabled`，不能使用 `Setting` 新增第二个 preference，也不能只改首页文案而不重载配置。
...
- 点击时先走现有 `PermissionUtil.requestFile`；获得权限后加载注册表、反转 `enabled`、保存原注册表，再重载 VOD 与已启用的直播配置。
- 新安装或没有显式自定义首页按钮列表的用户默认可见；已有自定义列表不被强制改写，仍可在“个性设置 → 首页按钮”中手动启用、隐藏和排序。
- 站点注入开关失败时保留原状态并提示既有权限/保存错误；不改变手机端完整编辑流程。
- 结论：实施。
...
- 不在按钮点击时只改内存对象：`CustomCspSetting.inject(...)` 在 VOD/直播配置加载阶段读取注册表，必须保存并触发配置重载，才能让启用/禁用对当前页面生效。
...
4. 在 `CustomCspSetting` 增加 `toggleEnabled()`，只负责读取、反转并保存同一注册表。
5. 在 TV `HomeActivity` 中处理权限、切换和错误；保存成功后重载 VOD 及满足现有条件的直播配置，并调用 `setFunc()`。
...
- [x] 点击按钮反转 `TV/CustomCsp/registry.json` 中的全局 `enabled`，而不是创建第二份状态。
- [x] 保存成功后重新加载当前 VOD/直播配置，使注入开关对当前页面生效。
- [x] 没有文件权限时沿用现有权限提示；保存失败时不刷新成错误状态。
...
- 定向源测试：`SiteInjectHomeButtonSourceTest`，覆盖新按钮 id/默认值、状态文案、共享注册表切换、权限入口、VOD/直播重载以及旧去广告入口移除。
...
- 设备场景（当前不可用）：首页聚焦“站点注入”→确认；有权限时按钮状态切换且当前 VOD/直播注入结果刷新；无权限时出现既有授权提示并保持状态；首页按钮设置中的隐藏/排序和左右焦点循环正常。
