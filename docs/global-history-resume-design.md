...
「历史记录跨源聚合」开启且 `Setting.isHistoryAggregationEffective()` 生效时，播放器不仅聚合历史卡片，也会在同一作品、同一标准季集之间共享进度。该规则同时适用于历史入口和用户从普通详情页显式选择的其他来源；关闭聚合时，只有明确的历史续播或跨源续播请求使用宽松集身份，普通显式选集继续按原始剧集身份判断。
